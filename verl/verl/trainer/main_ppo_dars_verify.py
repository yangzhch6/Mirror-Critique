...
from verl.trainer.ppo.ray_dars_verify_trainer import RayPPOTrainer
from verl.trainer.math_verify_reward import reward_fn_math_verify, reward_fn_math_verify_no_think, reward_fn_math_verify_no_think_no_code, reward_fn_true_false_
...
class RewardManager():
    ...
    def __call__(self, data: DataProto):
        ...
        judge_results_true_tensor = torch.zeros_like(data.batch['responses'], dtype=torch.float32)
        judge_results_false_tensor = torch.zeros_like(data.batch['responses'], dtype=torch.float32)
        ...
        def process_item(args):
            ...
            score, final_boxed_answer = compute_score_fn(solution_str=sequences_str, ground_truth=ground_truth)
            ...
            #         print(sequences_str)      
            return i, score, valid_response_length, final_boxed_answer
        ...
        for i, score, valid_response_length, final_boxed_answer in results:
            reward_tensor[i, valid_response_length - 1] = score
            if final_boxed_answer == "true":
                judge_results_true_tensor[i, valid_response_length - 1] = 1.0
            if final_boxed_answer == "false":
                judge_results_false_tensor[i, valid_response_length - 1] = 1.0

        return reward_tensor, judge_results_true_tensor, judge_results_false_tensor
...
@hydra.main(config_path='config', config_name='ppo_verify_trainer', version_base=None)
def main(config):
...
def main_task(config):
    ...
    from verl.trainer.ppo.ray_dars_verify_trainer import ResourcePoolManager, Role
