...
from verl.utils.fs import copy_local_path_from_hdfs, _is_non_local
...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def load_checkpoint(self, path=None, del_local_after_load=False, *args, **kwargs):
        ...
        if del_local_after_load:
            try:
                os.remove(local_model_path) if _is_non_local(local_model_path) else None
                os.remove(local_optim_path) if _is_non_local(local_optim_path) else None
                os.remove(local_extra_state_path) if _is_non_local(local_extra_state_path) else None
